...
namespace Triplet
{
    ...
    public class TokenizingControl : RichTextBox
    {
        ...
        public override string ToString()
        {
            ...
            List<object> inlines = para.Inlines.ToList<object>();
            string returnVal = "";
            foreach (object inline in inlines)
            {
                if (inline.GetType() == typeof(Run))
                    returnVal += (inline as Run).Text.Trim() + " ";
                else if (inline.GetType() == typeof(InlineUIContainer))
                    returnVal += (((inline as InlineUIContainer).Child as ContentPresenter).Content.ToString()) + " ";
            }
        ...
        private void ReplaceTextWithToken(string inputText, object token)
        {
            ...
            if (matchedRun != null) // Found a Run that matched the inputText
            {
                ...
                if (para.Inlines.Count == 1)
                    tokenContainer = CreateTokenContainer(inputText, token + "*");
